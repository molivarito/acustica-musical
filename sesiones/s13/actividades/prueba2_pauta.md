...
Rotulación según la regla fijada tras la revisión de alineamiento
2026-07-12 (P3): batidos/banda crítica/rugosidad = **OA2.2**;
escalas/temperamentos/cents = **OA2.3**; resonancia/impedancia/
acoplamiento/proporciones = **OA1.2**; mecanismos/regímenes/familias =
**OA1.3**.

| Ítem | OA principal | OA secundario | Pts | Nivel (regla 6b) |
|---|---|---|---|---|
| A.1–A.3 | OA3.1 | OA2.2, OA2.3 | 16 | transferencia (rúbrica) |
| B.1a | OA1.2 | OA4.1 (lectura de ejes, transversal) | 2 | básico |
| B.1b | OA1.2 | — | 2 | aplicación |
...
| C.1b | OA1.2 | — | 2 | **básico** (s12: $v/4L$, octava — regla 6b) |
| C.1c | OA1.3 | OA1.2 | 2 | **básico** (s12: abierto/cerrado por espectro) |
...
(B.3, C.1) entra SOLO con ítems básicos, como se anunció al cierre de
...
(220 y 330 Hz), sostenida ~4 s — **lisa, sin batido** —; silencio de
~1,5 s; la misma quinta con la nota superior **bajada ~10 cents**
(Mi4 ≈ 328 Hz): se oye una ondulación periódica clara, ~4 por segundo
(el 3.er armónico de la grave, 660 Hz, contra el 2.º de la aguda,
~656 Hz). Cambios audibles esperables: mismas dos alturas y misma
sonoridad global en ambas versiones; la segunda "ondula"/"late"/"se
frota" periódicamente; ningún oído debe reportar un cambio de altura
apreciable (10 cents está bajo el umbral melódico típico — esa es la
gracia del ítem).
...
|---|---|---|
| D1 descripción | "El mismo intervalo de dos notas sostenidas; las alturas no cambian entre versiones; en la 2.ª aparece una ondulación periódica de la sonoridad, ~4 por segundo, tipo batido; el conjunto se oye levemente más 'frotado'" | "La segunda suena peor / desafinada" (sin decir qué se oye distinto) |
...
componentes cercanos (batido entre fundamentales, entre armónicos, o
"dos fuentes casi iguales") con la cadena causa–efecto correcta puntúa
igual. La hipótesis "le agregaron un trémolo/vibrato electrónico"
describe el efecto sin mecanismo físico de superposición: D2 máximo 2,
salvo que la conecte explícitamente con la interferencia de dos
componentes (entonces es la misma hipótesis con otro nombre). La
hipótesis "cambió el timbre de los instrumentos" contradice el
enunciado (mismas fuentes, mismo intervalo): D2 máximo 2.

**Plan B** (si el audio de sala falló y se usó el estímulo en vivo —
dos flautas o dos tubos del taller al unísono, primero afinados y
luego uno destemplado levemente): anclas análogas — D1: misma nota,
ondulación periódica contable en la versión 2; D2: pequeña diferencia
de frecuencia entre las dos fuentes → batido $f_b = |f_2 - f_1|$;
D3: contar batidos / espectrograma con dos líneas casi superpuestas /
reafinar hasta detener el batido.
...
**D.1 (4)**: 0,5 pts por casilla.

| Instrumento | Energía | Válvula |
|---|---|---|
| Guitarra (pulsada) | de una vez | no hay (oscilación libre) |
| Piano (martillo) | de una vez | no hay (oscilación libre) |
| Violín (frotada) | continua | la fricción arco–cuerda (pega y suelta) |
| Flauta (soplada) | continua | el chorro de aire sobre el bisel (entra y sale) |

Se aceptan equivalentes claros ("stick-slip", "el chorro que oscila en
...
- Devolución en s14 **en la puerta al entrar** (precedente P2 de la
  ...
  proyectada durante los primeros minutos del módulo 1, corte duro;
  apelaciones por escrito en horario de consulta. No se resuelve ítem
  por ítem en aula: s14 tiene salida de medición y no hay holgura.

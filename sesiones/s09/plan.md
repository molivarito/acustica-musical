# Sesión 09 — Escalas y temperamentos: afinar de oído (batidos como herramienta)

**Objetivos que cubre**: OA2.3 (principal: fundamento perceptual de la
consonancia, las escalas y los temperamentos, demostrado audiblemente —
la actividad de afinación por conteo de batidos es literalmente su
evaluación declarada en diseno/01), OA1.2 (proporciones aplicadas:
razones de frecuencia y cents con aritmética simple; "para afinar el
objeto hay que cambiar un parámetro físico"), OA5.2 (continúa la serie
de talleres de medición sobre el objeto del proyecto, s08–s12).
Transversales: OA3.1 (escucha del día, con nota, pasada 2 de la
planilla) y OA4.1 (afinador y espectrograma bien leídos).
**Requisitos previos**: s08 (consonancia sensorial por coincidencia o
esquive de parciales; banda crítica; el ranking de lisura), s07 m2
(batidos contados; $f_b = |f_2 - f_1|$), s04 (tonos complejos,
inarmonicidad de cuerdas reales — reaparece en la afinación estirada).
El **ticket de salida de s08** ("si la 5ª perfectamente lisa es 3:2,
¿puede un piano tener TODAS sus quintas lisas a la vez? ¿qué habría
que sacrificar?") se cobra en la escucha del día: la respuesta es NO,
y el porqué es la coma pitagórica — de ahí nacen los temperamentos.
Los estudiantes traen: (a) **audífonos** (anunciado en s08), (b) **el
...
(d) leída la lectura previa `libro/cap09_escalas_y_temperamentos.md`.
...
afinar es siempre elegir un compromiso — audible, medible y contable
por batidos. Deja sembrada la resonancia (s10: hito 2 y puente hacia
los instrumentos).
**Reglas aplicadas y decisiones de carga (declaradas)**: escucha del
día (15′: estímulo 3′ + escritura 2′ + ronda de 4 sorteados × 2,5′ con
cronómetro visible) en el primer bloque del módulo 1; módulo 2 abre
con gancho sonoro sin ronda; la lectura previa se consume en la
mini-lección del módulo 1 (principio 6). Esta semana hay ronda + 2
talleres; se decide: (1) el **taller EVALUADO de la semana es la
afinación por batidos** (es la evaluación declarada de OA2.3 en
diseno/01; rúbrica rápida, cuenta en el 25 % de talleres); (2) el
taller del objeto ("¿en qué escala vive mi objeto?") se registra en la
**bitácora** y cuenta vía **hito 2** (espejo de la decisión de s08,
con los roles invertidos); (3) **sin instrumentos de estudiantes** (no
se avisó con una semana): la única guitarra de la sesión es la del
profesor, en demostración plenaria; (4) la escucha del día se genera
**en vivo con la demo** en modo presentación (sin estímulo nuevo para
el banco).

## Módulo 1 — La quinta imposible: de las razones a la coma (variante: experimental, adaptada)

| Tiempo | Bloque | Actividad del estudiante | Rol del profesor | Materiales |
|---|---|---|---|---|
| 0–15′ | **Escucha del día (OA3.1, con nota; OA2.3): se cobra el ticket de s08** | Escucha el estímulo (3′, dos pasadas): la MISMA quinta con tonos complejos, primero lisa y luego "casi igual pero que respira" (~1 ondulación/s); después la misma tercera mayor en dos versiones (una dulce, otra que muele rápido); todos escriben diagnóstico en 3 líneas (2′): describir la diferencia → hipotetizar el mecanismo (¿qué par de parciales se está pisando?) → proponer verificación; **4 sorteados** exponen (**2,5′ c/u, cronómetro visible**) | Opera `demo_temperamentos.html` en modo presentación (números ocultos), alternando afinación justa/igual sobre el mismo intervalo; evalúa con la planilla OA3 (pasada 2) premiando D1 (¿describe la ondulación LENTA de la 5ª vs. el batido RÁPIDO de la 3ª?) y D2 (¿invoca la coincidencia de parciales de s08?); lee 2–3 tickets de s08 y adelanta la respuesta sin desarrollarla: NO, un piano no puede tener todas sus quintas lisas — la mini-lección muestra por qué | `demo_temperamentos.html` proyectada (modo presentación); tickets de s08; planilla rúbrica OA3 (pasada 2); cronómetro |
| 15–30′ | **Mini-lección: la cuenta que no cierra (OA2.3, OA1.2)** — consume la lectura previa | Rehace con el profesor la aritmética del capítulo: intervalos = razones (2:1, 3:2, 4:3, 5:4, herencia directa del ranking de s08); apilar 12 quintas ≠ 7 octavas; con la regla nueva (cents: octava = 1200, semitono = 100, sumar intervalos = sumar cents): 12 × 702 = 8424 ≠ 8400 = 7 × 1200 → **sobran ~24 cents: la coma pitagórica (~23,5)**; responde votaciones: "¿la coma es un semitono entero, la mitad o un cuarto?" (≈ un cuarto), "¿dónde esconderían el error: todo en una quinta o repartido entre las 12?", "si reparto, ¿de cuánto es el 'impuesto' por quinta?" (2 cents) | Conduce la cuenta en el pizarrón con aritmética pura (sumas de cents, nada más); nombra el compromiso del **temperamento igual**: todas las quintas estrechadas ~2 cents — ninguna lisa, todas usables; anuncia el taller: "2 cents no se pueden 'oír' como desafinación… pero se pueden **contar**: la quinta temperada de Do4 late ~1 vez por segundo — ustedes van a afinar ambas quintas de oído, como un afinador de pianos (BEN 16.2)" | Pizarrón (la escalera de quintas vs. la torre de octavas); demo como apoyo (lectura de cents en pantalla) |
| 30–62′ | **Taller PEE: afinar de oído contando batidos (OA2.3, OA2.2, OA1.2) — formato 2, en parejas, EVALUADO** | En parejas dentro del grupo (2 parejas por notebook, por turnos): el **afinador** (audífonos) recibe de su operador una quinta desafinada al azar (botón de la demo, cents ocultos) y (A) la afina hasta dejarla **sin batidos**, anota los cents revelados; (B) desde ahí la **estrecha hasta contar ~1 batido/s**, anota los cents (¿cayó cerca de 700 = la quinta del piano?); luego intercambian roles. Fase C (parejas rápidas): repetir A con la 3ª mayor (batidos ~10 veces más rápidos: ¿se puede?). La pareja en espera resuelve los ítems de papel de la guía (predicción escrita ANTES del turno + la cuenta de la coma + conversiones de cents) | Rota por los grupos (~5′ c/u); vigila que la predicción esté escrita antes del primer turno y que los cents estén OCULTOS durante la afinación (se revelan al anotar); provoca: "¿su 'sin batidos' cayó en 702 o en 700 — y cómo lo sabe su oído sin mirar?", "¿por qué la 3ª es tan difícil de anular?", "¿qué haría el afinador de un piano real, que no tiene pantalla?" | 5 notebooks con `demo_temperamentos.html` (modo afinar, cents ocultos); audífonos personales; guía `guia_pee_afinar_por_batidos.md` (1 por pareja); cronómetro/reloj para contar batidos |
...
- **Taller del objeto (OA5.2 + OA1.2, a bitácora, cuenta vía hito 2)**:
  tabla de alturas en nota + cents con condiciones anotadas, veredicto
  de escala argumentado y UNA propuesta física de corrección de
  afinación — es el segundo insumo del compilado del hito 2 (s10).
- **Gancho cobrado**: la votación dividida del acorde en 3 afinaciones
  demuestra audiblemente que "afinado" es un compromiso — el corazón
  de OA2.3.
- **Cadena de tickets**: ticket s08 cobrado en la escucha del día
  (respuesta: NO — la coma); ticket s09 → gancho/escucha de s10
  (resonancia).

## Aritmética de taller declarada (regla 6b)

- **Módulo 1**: 15 + 15 + 32 + 8 = 70′ ✓. Escucha del día: 3′ + 2′ +
  4 × 2,5′ = 15′ ✓.
- **Taller de afinación (m1, 30–62′)**: 32′ = 2′ instalación + 30′ de
  trabajo; 1 notebook/grupo ÷ 2 parejas = 15′ por pareja; cada
  afinador hace las fases A y B en **~6′ de oído por persona** (A ~3′,
  B ~2′, anotación ~1′) ✓ ≥2′; la pareja en espera trabaja los ítems
  de papel de la guía (nadie mira sin hacer, principio 3); fase C
  declarada opcional (solo parejas rápidas).
...
- **Demo** (skill /demo-interactiva): `demo_temperamentos.html` — modo
  comparar (mismo intervalo/acorde en justa, pitagórica y temperamento
  igual; selector 3ª mayor / 5ª / acorde mayor; razones y cents a la
  vista; tonos complejos de 6 armónicos; vista de parciales con el par
  en colisión y sus batidos previstos) + modo afinar (nota superior
  ajustable en cents, cents ocultables, botón "desafinar al azar") +
  modo presentación para la escucha del día. Es la única demo de la
  sesión.
- **Apuntes**: `sesiones/s09/apuntes/apunte_s09_escalas_y_temperamentos.md`.
- **Capítulo (lectura previa)**: `libro/cap09_escalas_y_temperamentos.md`.
- **Actividades**: guía PEE evaluada `guia_pee_afinar_por_batidos.md`
  ...
  `sesiones/s09/actividades/`.
- **Trabajo previo del profesor (esta semana)**: probar
  `demo_temperamentos.html` en el equipo de la sala (alternancia
  justa/igual del estímulo en modo presentación: la ondulación de la
  ...
  6 guías de objeto; planilla OA3 con el sorteo de la pasada 2;
  recordatorio por correo: **audífonos + objeto del proyecto +
  afinador instalado**.
...
- **La ondulación de la quinta temperada no se oye en la sala**
  (parlantes o ruido de fondo) → subir el registro del estímulo (base
  más aguda = batidos proporcionalmente más rápidos y más notorios) o
  abrir la escucha con la 3ª mayor (batido ~10/s, robusto) y dejar la
  quinta como segunda pasada; el taller no se afecta (es con
  audífonos).
- **Audífonos olvidados** → la pareja afina con el parlante del
  notebook a volumen bajo (los batidos de la quinta sobreviven bien);
  si faltan muchos, la fase B (contar ~1 batido/s) se hace plenaria
  con la demo proyectada y cada pareja solo hace la fase A en su
  turno.
- **La demo no corre en los notebooks de grupo** → plan B del curso:
...
- **Un grupo llega sin su objeto** → variante de la guía: medir la
  ...
  es la entrega).
- **El objeto no tiene altura definida** (espectro inarmónico, la app
  "duda") → la guía lo contempla: reportar la altura más plausible y
  la duda de la app ES el resultado (conecta con la radiografía de
  s08: sin parciales armónicos no hay nota nítida); el veredicto pasa
  a ser "mi objeto no vive en ninguna escala, y sabemos por qué".
- **La fase C tienta a alargar el taller** → está declarada opcional;
  a los 62′ el pizarrón cambia a mapa de cents aunque haya parejas a
  medio camino (fusible del módulo 1).
- **La guitarra no coopera** (armónicos débiles en la sala) → acercar
  el micrófono de la sala a la boca de la guitarra o, en el peor caso,
  reemplazar por la demo (dos "cuerdas" sintéticas en 4:3 desafinadas
  2 cents): se pierde el objeto real, no el fenómeno.

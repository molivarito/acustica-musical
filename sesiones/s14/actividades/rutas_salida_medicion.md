# Rutas de la salida de medición — PLANTILLA EDITABLE (s14, módulo 2)

**Estado**: plantilla. Los espacios concretos, permisos y tiempos de
tránsito **los fija el profesor** antes de imprimir [POR COMPLETAR:
nombres reales de espacios UC del campus, según permisos y distancia a
pie desde la sala]. Criterios de diseño que las rutas deben respetar:

- **Contraste garantizado**: cada ruta combina al menos un espacio
  SECO (sala de clases chica, sala de práctica/estudio, oficina) y uno
  VIVO (hall o pasillo de hormigón, caja de escala, baño embaldosado,
  capilla/auditorio). Sin contraste no hay informe comparativo.
- **Todo a ≤5 min a pie** de la sala (el presupuesto de tránsito por
  espacio es ~2–3′); rutas bajo techo o con tramos cubiertos (plan
  lluvia).
- Espacios **A y B son obligatorios (★)**; el C es opcional (solo si
  la ruta va al día). Cada ruta declara un **espacio alternativo** por
  si algo está ocupado o cerrado.
- Entre las 5 rutas conviene repetir 1–2 espacios "ancla" (p. ej. que
  dos rutas midan el mismo hall): las mediciones cruzadas de grupos
  distintos en el mismo espacio son oro para la puesta en común
  (¿reproducibilidad entre celulares?).
- La **capilla o el auditorio** requieren permiso/reserva previa
  [POR VERIFICAR: disponibilidad y autorización]; si no se consigue,
  la caja de escala de un edificio es un sustituto vivo excelente.

**Hora de corte** (escribirla en el pizarrón y en cada guía): minuto
55 del módulo — todos caminando al punto de encuentro aunque falte el
espacio C. **Punto de encuentro**: [POR COMPLETAR: el aula u otro].

## Ruta 1 — [nombre]

| Orden | Espacio | Tipo esperado | Notas |
|---|---|---|---|
| A ★ | [sala de clases pequeña — puede ser la propia] | seco | |
| B ★ | [hall / pasillo de hormigón del edificio X] | vivo | fondo alto probable: extrapolar |
| C (opcional) | [sala de práctica / estudio] | muy seco | |
| Alternativa | [espacio cercano] | | usar si A/B/C ocupado |

## Ruta 2 — [nombre]

| Orden | Espacio | Tipo esperado | Notas |
|---|---|---|---|
| A ★ | [sala de práctica / estudio de música] | muy seco | reservar franja |
| B ★ | [caja de escala del edificio X] | vivo | globo lejos de oficinas |
| C (opcional) | [pasillo ancho / hall chico] | medio | |
| Alternativa | [espacio cercano] | | |

## Ruta 3 — [nombre]

| Orden | Espacio | Tipo esperado | Notas |
|---|---|---|---|
| A ★ | [capilla / auditorio — requiere permiso] | muy vivo | palmada si hay gente; permiso [POR VERIFICAR] |
| B ★ | [sala de clases mediana] | seco | |
| C (opcional) | [hall de acceso] | vivo | ancla compartida con ruta 5 |
| Alternativa | [espacio cercano] | | |

## Ruta 4 — [nombre]

| Orden | Espacio | Tipo esperado | Notas |
|---|---|---|---|
| A ★ | [biblioteca — zona permitida, SOLO palmada] | seco (techo bajo, alfombra) | coordinar con biblioteca |
| B ★ | [baño embaldosado / camarín] | vivo (¡la ducha del ticket!) | grupo chico adentro, puerta abierta |
| C (opcional) | [pasillo del mismo edificio] | medio | |
| Alternativa | [espacio cercano] | | |

## Ruta 5 — [nombre]

| Orden | Espacio | Tipo esperado | Notas |
|---|---|---|---|
| A ★ | [sala de clases grande / sala de conferencias] | medio | |
| B ★ | [hall de acceso del edificio X] | vivo | ancla compartida con ruta 3 |
| C (opcional) | [oficina / sala de reuniones chica] | muy seco | pedir 5 min |
| Alternativa | [espacio cercano] | | |

## Checklist del profesor (semana previa)

- [ ] Fijar los 15 espacios (5 rutas × 3) + alternativas; caminar las
      rutas una vez cronometrando.
- [ ] Permisos: capilla/auditorio, biblioteca, salas de práctica
      [POR VERIFICAR cada uno].
- [ ] Medir el $T_{60}$ de UN espacio con la app elegida (ensayo del
      método y valor de control).
- [ ] Crear la planilla compartida (1 fila por espacio × grupo:
      espacio, fondo, mediana $T_{60}$, V, impulso) y probar el enlace
      desde un celular.
- [ ] Globos (bolsa de ~25: 4 por grupo + repuestos) e inflarlos con
      los grupos ANTES de salir (inflar en la calle quita tiempo).
- [ ] Imprimir 5 guías + 5 rutas; escribir hora de corte y punto de
      encuentro en ambas.

# Hoja de registro individual — El mapa del choque (s08, módulo 1, 22 min)

**Objetivos**: OA2.2 (medir las zonas batido/rugosidad/separación en el
propio oído). **Esta hoja se entrega al salir del módulo y NO lleva
nota**: es su dato psicoacústico personal y evidencia del curso (se
archiva; la reencontrará en s09 al afinar por batidos).
**Formato**: en parejas dentro del grupo (2 parejas por notebook, por
turnos de ~10′ por pareja). Roles en cada turno: **oyente** (audífonos,
dicta SUS fronteras — no se negocian) y **operador** (mueve Δf despacio
y anota lo que el oyente dicte). Luego intercambian. La pareja que
espera trabaja los ítems 1 y 4 de esta hoja.
**Materiales**: notebook con `demo_banda_critica.html` (tonos puros,
f₁ = 440 Hz; si quieren contar batidos finos, `demo_batidos.html` de
s07 sirve igual), audífonos personales, esta hoja (una POR PERSONA).

Nombre: ____________________ Pareja: ____________________

## 1. Predicción individual (ANTES de su turno de oyente)

- "Creo que dejaré de poder contar batidos alrededor de Δf ≈ ____ Hz."
- "Creo que la aspereza desaparecerá del todo alrededor de
  Δf ≈ ____ semitonos."

## 2. Medición (su turno de oyente, ~5 min)

El operador parte del unísono (Δf = 0) y sube MUY despacio. Usted
dicta "alto" en cada frontera; el operador lee Δf en Hz y semitonos en
la pantalla y usted lo anota. Si duda, pida bajar y volver a cruzar la
frontera (por arriba y por abajo suele dar distinto: anote el punto
medio).

| Frontera (con f₁ = 440 Hz) | Δf (Hz) | Δf (semitonos) |
|---|---|---|
| **F1** — hasta aquí puedo CONTAR batidos | | |
| **F2** — desde aquí oigo DOS notas (aunque rocen) | | |
| **F3** — desde aquí el par queda LISO | | |

Marque también sus tres fronteras con los botones de la demo: el mapa
de zonas de la pantalla es su registro visual (captúrelo si alcanza).

**Solo grupos rápidos**: repita F3 con f₁ = 220 Hz.
F3(220 Hz) = ____ Hz = ____ semitonos. ¿Se mantuvo en Hz o en
semitonos? ______________________

## 3. Contraste (al terminar los dos turnos de la pareja, 2 min)

- ¿Se cumplió F1 < F2 < F3? ¿Sus fronteras coinciden con las de su
  pareja o hay diferencias claras? ¿En cuál frontera discreparon más?
  ________________________________________________________________
- Su predicción del ítem 1, ¿le achuntó al orden de magnitud?  Sí / No

## 4. Proyección (mientras espera turno; se discute en la síntesis)

- El espectrograma de la app "ve" dos líneas limpias desde Δf = 1 Hz,
  pero usted oyó UNA nota hasta F2. Escriba una frase que empiece con:
  "La rugosidad no es una propiedad del sonido, porque..."
  ________________________________________________________________
- Regla de conversión de la hoja: cerca de 440 Hz, **1 semitono ≈ 6 %
  de f₁ ≈ 26 Hz**. Su F3 en Hz, ¿a qué intervalo musical se parece
  (semitono, tono, tercera menor…)? _____________________________

## Cierre en aula

Al pasar el vocero, cada integrante aporta su F3 en semitonos a la
recta del pizarrón (el "mapa del curso"). Entregue esta hoja al salir
del módulo: se archiva sin nota. El taller evaluado de esta semana es
la radiografía espectral del módulo 2.

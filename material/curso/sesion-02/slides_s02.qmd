...
## Escucha del día — la práctica de cada semana {.smaller}
...
<audio controls preload="auto" src="../../estimulos/e02_flauta_ataque_soploso.wav" style="width: 70%; margin-top: 0.5em;"></audio>

::: {.notes}
Estímulo: `e02_flauta_ataque_soploso.wav` (nota C4, ~5,5 s), dos pasadas — el reproductor está aquí mismo en la slide.
...
::: {.notes}
**Puente desde la escucha** (el plan lo deja implícito): "¿Se fijaron
en el arranque de la flauta de recién, ese soplido antes de que la
nota enganche? Eso es un golpe. Y justamente la semana pasada ustedes
predijeron en su ticket cómo se vería un golpe dibujado — hoy lo
comprobamos." La pregunta del golpe viene de SUS tickets de s01, no
de la escucha; la flauta fue elegida porque su ataque contiene uno.
...
::: {.notes}
**Apoyo:** Esta slide es TODO lo que el taller que viene necesita del
espectro — enseñarlo aquí, en 3 minutos, con dos ejemplos polares:
un **silbido** ≈ un solo peak angosto (una frecuencia domina); una
**"sss"** ≈ meseta ancha sin peaks (muchas frecuencias a la vez, sin
altura). Con esos dos extremos, la vocal y el golpe del taller se
extrapolan. Tranquilidad PEE: las predicciones del taller PUEDEN
salir toscas o erradas — el aprendizaje está en el contraste
dibujo-vs-pantalla, y la rúbrica premia predecir por escrito y
corregir, no acertar. La lectura previa (cap. 2) ya les presentó
esto: aquí solo se formaliza.
:::
...
## Un mismo sonido, tres retratos

![](../../assets/figuras/s02_tres_retratos.svg){fig-alt="Una misma señal en sus tres representaciones: forma de onda, espectro y espectrograma" width="92%"}

::: {.notes}
**Apoyo:** Es la Figura 1 del apunte (la lectura ya la trae): una misma
señal — un tono con cuatro parciales, como vocal cantada — en sus tres
vistas. Úsela para cerrar la mini-lección señalando: en la forma de
onda, el golpe inicial es la sacudida breve (la "mancha" del voto); en
el espectro, la peineta de parciales; en el espectrograma, líneas
horizontales y el golpe como columna vertical. La versión EN VIVO
viene en la demo siguiente: golpe seco en la mesa frente al micrófono
(forma de onda de un golpe real) y un silbido (peak angosto en el
espectro) — modelar esos dos antes de repartir las guías.
:::
...
vivo, proyectada. Modelar la lectura de las dos vistas con DOS sonidos
que no arruinan el taller: un **golpe en la mesa** (su "línea o
mancha" ya quedó revelada por el voto) y una **nota de la flauta**
(no está entre los 4 del taller, conecta con la escucha de la mañana:
peineta de parciales + neblina de soplo). NO modelar aaa, sss ni
silbido — esos retratos quedan vírgenes para las mesas; el aaa/ooo de
la slide anterior se afirma y se deja como intriga: la mesa que
produzca la "aaa" lo comprueba en el taller. Si el micrófono falla,
conmutar a la fuente interna (oscilador) de la demo.
**Para el golpe usar «🔴 Capturar golpe»** (no "Congelar" a mano, que
nunca llega a tiempo): pulsar, golpear, y a los 5 s la demo congela
sola la forma de onda completa (ataque y extinción, eje en segundos) y
el espectro de máximos del golpe. «Descongelar» vuelve al vivo. Lo
mismo en el taller cuando le toque el golpe a una mesa.
:::
...
<audio controls preload="auto" src="../../estimulos/e03_trueno_retardo.wav" style="width: 70%; margin-top: 0.5em;"></audio>
...
velocidad)?"): con v ≈ 343 m/s, 3 segundos de retraso
equivalen a cerca de 1 kilómetro de distancia (con 3,5 s medidos:
~1,2 km). Tras la puesta en
común, instalar v ≈ 343 m/s a 20 °C como dato del curso y
...
::: {.notes}
**Apoyo:** Este valor (v ≈ 343 m/s a 20 °C) se instala como
...
"¿Por qué su app no muestra nada sobre 22 000 Hz?": con fₛ
muestras por segundo solo se representa fielmente lo que está bajo
fₛ/2; con 44 100 muestras/s, nada sobre 22 050 Hz). Si las apps son
...
## La nota de la mañana, con ojos nuevos
...
::: {.notes}
**Apoyo:** (Reubicada al cierre tras la s02 de 2026: entre la teoría
de v = λf y el taller quedaba fuera de contexto.) Después de la
puesta en común de los grupos, cerrar releyendo la MISMA nota de
flauta de la escucha de la mañana, ahora que saben mirar: las
**líneas horizontales parejas** son el tono que describieron; la
**neblina entre las líneas** es el soplo que llamaron "aire". Lo que
en la mañana diagnosticaron de oído, en la tarde lo leen en el
instrumento que traen en el bolsillo — ese es el arco del día, y
alimenta directo el ticket: si esta nota son líneas… ¿qué serán las
del vaso golpeado?
:::

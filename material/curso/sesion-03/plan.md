# Sesión 03 — Modos de vibración: por qué cada objeto suena como suena

**Objetivos que cubre**: OA1.1 (principal: identificar modos y parciales
...
(notación de `PLAN_SEMESTRE.md`) y el caso percusión/modos 2D (decisión D4: este es
SU lugar en el curso). s04 usa los modos de la cuerda de inmediato (receta
del timbre, punto de excitación). Desde hoy corre en paralelo el proyecto
(OA5): hito 1 en s05.
...
| 25–60′ | **Taller PEE: la sartén y sus parientes (OA1.1, OA1.2)** — formato 2, kit casero | En grupo, con roles y la guía: elige 2 objetos metálicos del kit (sartén, tapa de olla, taza…); ANTES de golpear predice por escrito: nº de líneas, ¿espaciado regular?, ¿cuál decae primero?; golpea y mide con la app: anota las frecuencias de los 3 parciales más fuertes y calcula sus razones ($f_2/f_1$, $f_3/f_1$); fase 2: predice y verifica el efecto de (a) golpear en otro punto y (b) apoyar un dedo en el centro vs. en el borde | Rota por los grupos (~6′ c/u); provoca: "¿las razones dan números enteros?", "¿el dedo apaga TODO el sonido o solo parte?"; recuerda que el contraste predicción–resultado vale tanto como el acierto | 3 kits: sartén u olla pequeña, tapa metálica, taza o vaso, cuchara de palo + celulares con app de espectrograma (mín. 2 por grupo); guía PEE "la sartén" (1 por grupo) |
| 60–70′ | Cierre y contraste | UN grupo expone su tabla de razones y su hallazgo del dedo; todos anotan la moraleja en la guía: cada objeto tiene su familia fija de frecuencias características; dónde se golpea y dónde se toca decide cuáles suenan | Sintetiza en el pizarrón con las razones de los 3 grupos: ninguna da 1 : 2 : 3 (objetos inarmónicos); planta la pregunta puente al módulo 2: "¿qué objeto tendría parciales en razones enteras?"; cierra las guías con rúbrica rápida | Pizarrón; guías |
...
| 40–65′ | **Lanzamiento del proyecto del curso (OA5.1)** — hito del semestre | Recibe y lee el enunciado (10′ de presentación); trabaja 15′: cada estudiante piensa y escribe SU propia media hoja con 2 ideas candidatas de SU objeto (instrumento/instalación), cada una con: qué modelo del curso permitiría *predecir* su sonido y qué se podría *medir*; comparte brevemente con su mesa, que opina pero no decide; entrega su media hoja individual (sin nota: insumo para feedback en s04) | Presenta el enunciado completo: qué es (proyecto **individual** — cada estudiante persigue su propio objeto; la mesa es equipo de apoyo, no dueña del proyecto), 3 hitos con fechas y % (s05 diseño con predicción 10% · s10 avance 10% · s15 presentación + informe ≤6 págs 15%), regla de oro ("la explicación vale más que el éxito sonoro"), bitácora **individual** desde hoy, talleres de medición s08–s12 sobre el objeto propio con la mesa como apoyo; luego rota por las mesas empujando a cada estudiante hacia ideas medibles y realistas | Enunciado impreso (1 por estudiante) + proyectado; 9 medias hojas (1 por estudiante) |
...
- **Chequeo de la distinción parcial/armónico** (mínimo de OA1.1): al
  cerrar el módulo 2, pregunta relámpago a mano alzada — "el tercer
  parcial de la sartén ¿es un armónico?" — debe responderla la sala
  completa sin dudar.
- **Media hoja de ideas de proyecto** individual (9, OA5.1, sin nota):
  evidencia de que el lanzamiento produjo ideas propias; la mesa opinó
  pero no decidió; el profesor la devuelve comentada en s04.
- **Ticket de salida** hacia s04 (punto de pulsación: ¿nota o timbre?):
  siembra la "receta del timbre" y se retoma al abrir s04.
...
- **Material físico/logístico**: 3 kits de objetos metálicos (sartén u
  ...
  monocordio (elástico grueso + caja como plan B); grabaciones cortas de
  timbal y tom (banco de estímulos, pendiente 7); imágenes de figuras de
  Chladni con licencia adecuada; enunciado del proyecto impreso; tickets
  de s02 recopilados y ordenados por grupo.
- **Pendiente para s04**: pauta breve de feedback de las medias hojas de
  ideas (el profesor las devuelve comentadas).
...
- **Las apps no resuelven los parciales de la sartén** (parciales muy
  ...
  pantalla justo tras el golpe; plan B por kit: la tapa colgada de un
  cordel (suena más largo); mínimo operativo: 3 parciales legibles por
  objeto.
- **Taller de cuerda no alcanza** (regla de menos contenido) → decisión
  ya tomada: los modos de cuerda van en demo predictiva (10–25′ del
  módulo 2) y el taller manual de cuerda visible queda para s04 con los
  instrumentos de los estudiantes; si el módulo 2 se atrasa, se recorta
  la mini-lección 2D a 10′ (las figuras de Chladni pasan al apunte) y
  NUNCA el lanzamiento del proyecto.
- **El lanzamiento del proyecto se come la discusión** (estudiantes que
  piden más tiempo de ideas) → el trabajo de ideas sigue por bitácora
  durante la semana; la media hoja puede entregarse hasta el inicio de
  s04 sin penalización; lo innegociable hoy es que el enunciado quede
  entendido (hitos, fechas, regla de la explicación, proyecto
  individual con mesa de apoyo).
- **No hay guitarra a mano para el gancho** → monocordio casero (elástico
